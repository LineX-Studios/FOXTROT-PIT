...
public class CommandFoxtrot extends CommandBase {
    ...
    public String getCommandUsage(ICommandSender sender) { 
        return "/foxtrot <add|remove|list|alerts|toggle|clear|denick|debug|esp|autodenick|hud>"; 
    }
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (args.length == 0) {
            ...
            sender.addChatMessage(new ChatComponentText(EnumChatFormatting.YELLOW + "/foxtrot toggle " + EnumChatFormatting.GRAY + "- Toggle HUD"));
            ...
            sender.addChatMessage(new ChatComponentText(EnumChatFormatting.YELLOW + "/foxtrot clear " + EnumChatFormatting.GRAY + "- Clear list"));
            return;
        }
        ...
        switch (action) {
            ...
            case "denick":
                if (args.length > 1) {
                    ...
                    sender.addChatMessage(new ChatComponentText(EnumChatFormatting.GRAY + "[" + EnumChatFormatting.RED + "PIT" + EnumChatFormatting.GRAY + "] " + EnumChatFormatting.RED + "Usage: /fx denick [name]"));
                }
                break;
            ...
            case "add":
                if (args.length > 1) {
                    ...
                    if (!EnemyHUD.targetList.stream().anyMatch(name -> name.equalsIgnoreCase(nameToAdd))) {
                        ...
                        sender.addChatMessage(new ChatComponentText(EnumChatFormatting.GRAY + "[" + EnumChatFormatting.RED + "PIT" + EnumChatFormatting.GRAY + "] " + 
                            EnumChatFormatting.GREEN + "Added " + nameToAdd + " to enemies."));
                    } else {
            ...
            case "remove":
                if (args.length > 1) {
                    ...
                    if (removed) {
                        sender.addChatMessage(new ChatComponentText(EnumChatFormatting.GRAY + "[" + EnumChatFormatting.RED + "PIT" + EnumChatFormatting.GRAY + "] " + 
                            EnumChatFormatting.RED + "Removed " + nameToRemove + " from enemies."));
                    } else {
                        sender.addChatMessage(new ChatComponentText(EnumChatFormatting.GRAY + "[" + EnumChatFormatting.RED + "PIT" + EnumChatFormatting.GRAY + "] " + 
                            EnumChatFormatting.YELLOW + nameToRemove + " was not found on your list."));
                    }
                } else {
            ...
            case "toggle":
                ...
                ConfigHandler.saveConfig();
                sendMessage(sender, EnumChatFormatting.YELLOW + "Toggle HUD: " + (EnemyHUD.enabled ? EnumChatFormatting.GREEN + "ON" : EnumChatFormatting.RED + "OFF"));
                break;
            ...
            case "autodenick":
                ...
                ConfigHandler.saveConfig();
                sendMessage(sender, EnumChatFormatting.YELLOW + "AutoDenick: " + 
                    (AutoDenick.enabled ? EnumChatFormatting.GREEN + "ON" : EnumChatFormatting.RED + "OFF"));
                break;
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) return getListOfStringsMatchingLastWord(args, "add", "remove", "list", "alerts", "toggle", "clear", "denick", "debug", "esp", "autodenick", "hud");
        if (args.length == 2 && (args[0].equalsIgnoreCase("add") || args[0].equalsIgnoreCase("denick"))) {
            return getListOfStringsMatchingLastWord(args, Minecraft.getMinecraft().getNetHandler().getPlayerInfoMap().stream().map(info -> info.getGameProfile().getName()).collect(Collectors.toList()));
        }
