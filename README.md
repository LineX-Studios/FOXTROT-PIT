...
<img width="535" height="194" alt="image" src="https://github.com/user-attachments/assets/862b1741-951f-4ea6-8108-a0cdfa193ee9" />
    ...
    Player ESP: Red for enemies, green for friends.
...
<img width="504" height="366" alt="image" src="https://github.com/user-attachments/assets/53de828b-af5f-4e3e-8c9d-3b9914005db5" />
    ...
    PIT ESP: Zero-delay Dragon Egg ESP (highly optimized to prevent lag in packed lobbies), Sewer Chests, Raffle Tickets, and dropped Mystics.
...
USE AT YOUR OWN RISK, WE ARE NOT LIABLE IF THE AUTO USE MODULES HAVE RESULTED IN A BAN ESPECIALLY WITH AUTO PODS AND AUTO BULLET TIME IT CAN RESAULT IN A BAN NORMAL PANT SWAP MIGHT NOT BE FULLY SAFE EITHER BUT IT WORKS VERY WELL ON HYPIXEL, BUT USE AT YOUR OWN RISK
    ...
    Auto Clicker: Highly randomized, hold-to-click, fast place, and an item whitelist to prevent accidentally clicking WITH NON WHITELISTED ITEMS.
    ...
    Auto Swaps: Hover and hold right-click to fast-swap pants. Auto Venom Swap equips diamond pants instantly upon getting venomed.

    Combo Escapes: Auto Escape Pod and Auto Bullet Time to quickly get out of bad situations.

    Misc: Auto Quick Math Solver, Toggle Sprint.
...
<img width="1081" height="133" alt="image" src="https://github.com/user-attachments/assets/6b582577-f34f-40cc-8c9d-9b732a5de773" />
    ...
    Hooks into the PitPal API to resolve nicked players using their mystic item nonces.

    To prevent rate-limiting and client freezes, the scanner ignores fresh items, rage pants, and darks. It only scrapes in the background once a player actively holds a valid mystic sword or armor piece.

Installation

    Download Foxtrot.jar from the Releases tab.

    Drop it into your .minecraft/mods folder.

    Launch Minecraft using Forge 1.8.9.

Compiling from Source

If you want to build the project yourself, you must use JDK 8 (1.8). Newer versions (like jdk 11 or 17) will cause ForgeGradle to crash.

    Clone the repository.

    USING GIT BASH - git clone https://github.com/LineX-Studios/Foxtrot.git

    Run gradlew build to compile.

    USING GIT BASH - ./gradlew build

    Your finished mod will be located in the build/libs/ directory. select foxtrot.jar not foxtrot-sources.jar

Disclaimer: Use at your own risk. Includes Features like AutoClicker and AutoUse Features that violate the rules of most Hypixel and may result in a ban.
